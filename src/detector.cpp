...
//#define detector::max(a, b) (((a)>(b)) ? (a) :(b))
//#define detector::min(a, b) (((a)<(b)) ? (a) :(b))
...
//bool Detector::mode_flag = false;

void Detector::init(std::string def, std::string net,
...
void Detector::setComputeMode(std::string mode, int id)
{
    if (mode == "gpu")
    {
        Caffe::SetDevice(id);
...
std::vector<Detection> Detector::detect(cv::Mat img, float conf_thresh, float nms_thresh)
{

    setThresh(conf_thresh, nms_thresh);
    int rpn_num;
    float *pred = NULL;
    if (img.empty())
    {
        /*logic_error ex("未设置图片");
...
//    {
//        t2 = CURTIME()->gettime();
//        cout << "1 Copy time:" << t2 - t1 << endl;
//    }

    //TODO:暂设置
    //width = 1056; height = 640;
    //im_scale_x = std::floor(width) / floor(img.cols);
    //im_scale_y = std::floor(height) / floor(img.rows);
...
//    {
//        t1 = CURTIME()->gettime();
//        imwrite("cv_resize.jpg", cv_resized);
//        cout << "resized :" << width << "," << height << endl;
//    }
...
std::vector<Detection> Detector::detect(cv::Mat img, std::string class_mask, float conf_thresh, float nms_thresh)
{
    std::vector<Detection> dets = detect(img, conf_thresh, nms_thresh);
...
std::vector<Detection> Detector::detectLowResolution(cv::Mat img, float resolution_reduce,
                                                     float conf_thresh, float nms_thresh)
{
    ...
    for (int i = 0; i < dets.size(); ++i)
    {
        cv::Rect tmp_rect = dets[i].getRect();
        tmp_rect.x = tmp_rect.x * resolution_reduce;
        tmp_rect.y = tmp_rect.y * resolution_reduce;
        tmp_rect.width = tmp_rect.width * resolution_reduce;
        tmp_rect.height = tmp_rect.height * resolution_reduce;
        dets[i].setRect(tmp_rect);
        mask_dets.push_back(dets[i]);
    }
    if (!mask_dets.size())
        mask_dets = std::vector<Detection>();
...
std::vector<Detection> Detector::detectLowResolution(cv::Mat img, std::string class_mask,
                                                     float resolution_reduce,
                                                     float conf_thresh, float nms_thresh)
{
    std::vector<Detection> dets = detect(img, conf_thresh, nms_thresh);
    std::vector<Detection> mask_dets;
...
void Detector::drawBox(cv::Mat img, std::vector<Detection> dets)
{
    for (int i = 0; i<dets.size(); ++i)
    {
        //cout << car_dets[i].getClass() << " " << car_dets[i].getRect() << " " << car_dets[i].getScore() << endl;
        rectangle(img, dets[i].getRect(), cv::Scalar(255, 0, 0), 4);
        std::ostringstream os;
        os.precision(2); //保留2位有效数字
        os << dets[i].getScore();
        /*string showTxt = car_dets[i].getClass() + "," + os.str();*/
        std::string showTxt = dets[i].getClass();
        putText(img, showTxt, // Mat, text
            cv::Point(dets[i].getRect().x, dets[i].getRect().y - 2), //oriPt
            cv::FONT_HERSHEY_SIMPLEX, 0.7, //Font
            cv::Scalar(0, 0, 255), 2); // color, linear width
    }
}
...
void Detector::nms(int* keep_out, int* num_out, const float* boxes_host, int boxes_num,
...
vector<Detection> Detector::overThresh(int* keep, int num_out, float* sorted_pred_cls, float CONF_THRESH, string className)
{
    vector<Detection> dets;
    int i = 0;
    while (i < num_out)
