...
public:
	...
	std::vector<Detection> detect(cv::Mat img, float conf_thresh = 0.7, float nms_thresh = 0.3);
	//得到给定类别的目标
	std::vector<Detection> detect(cv::Mat img, std::string class_mask, float conf_thresh = 0.7, float nms_thresh = 0.3);
    std::vector<Detection> detectLowResolution(cv::Mat img, float resolution_reduce,
                                               float conf_thresh = 0.7, float nms_thresh = 0.3);
	std::vector<Detection> detectLowResolution(cv::Mat img, std::string class_mask,
											   float resolution_reduce,
											   float conf_thresh = 0.7, float nms_thresh = 0.3);
//	void detect_aux(cv::Mat img, float* pred, int& rpn_num);
	//显示函数
